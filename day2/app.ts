...
const menu = document.querySelector("ul.menu") as HTMLDivElement;
const renderMenu = () => {
...
function addToCartOnClick(index: number) {
...
const cart = document.querySelector("ul.cart-summary") as HTMLDivElement;

function renderCart(cartItems: MenuItem[]) {
...
const tax = document.querySelector(".amount.price.tax") as HTMLDivElement;
const total = document.querySelector(".amount.price.total") as HTMLDivElement;
...
//eg 223 to 2.23
function convertToDollarsAndCents(total: number): number {
  const dollars = Math.floor(total / 100);
  const cents: number = Math.round(total % 100);

  let dollarsAndCents = "";
  if (cents < 10) dollarsAndCents = dollars + "." + "0" + cents;
  else dollarsAndCents = dollars + "." + cents;
